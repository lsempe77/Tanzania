...
The report focuses on the results of RBF answering the following evaluation questions:

  + Was there evidence of improved results to which RBF contributed? 
  
  + To what degree, and in what ways?  
  
  + Has behavior change occurred, where and to what extent? Have changes been sustained?
...
The analysis is structured in three main sections, followed by an Appendix. The first section describes contextual and payment data. The second section focuses on descriptive and inferential analysis of data related to DLI 4.2., while the third section approaches DLI 6.2. 
...
- `payments_alldlrs.c`: Table \@ref(tab:pay2) (in the next section) shows the evolution of DLI 4.2. payments to LGAs by year. Table \@ref(tab:pay3) aggregate DLI 4.2 payments for all years of the period considered (losing time dimension).  
...
autofit(pay1)
set_caption(pay1,"Total LGA DLI Payment per year")
...
This section compares the PTR with EP4R payments made on DLI 4.2; discusses whether there was an improvement in teacher deployment, and at what level/ in what senses teacher deployment improved.
...
DLI payments are aggregated at council and DLI (losing the time dimension) to use in the analysis of DLI 4 - PTR (using only data on 2015 and 2019, a before/after picture of the payable part of the program). Table \@ref(tab:pay3) shows that 151 councils received payments over the period, while 33 did not. Table \@ref(tab:pay2) shows evolution of DLI 4.2. payments over time, where year 2018 shows the highest council average: $\$33,880$  $(s.d. \$65,192)$, and 102 councils received resources.
...
set_caption(pay2,"DLI 4.2. payments per council, US$")
...
set_caption(pay3,"Councils receiving money for DLI 4.2 over the four years 2016-2019")
...
```{r, pay4, fig.cap="Histogram DLI 4.2. payments per council, combined 2016-2019, $ per council"}
...
set_caption(sum, "PTR descriptive statistics, school count and PTR (columns 2 to 4)")
...
Figure \@ref(fig:quartiles) shows school-level data for pupil-teacher ratios across years 2015, 2017 and 2019. Colors represent each region. Outliers are shown as those that are at more than 1.5 units from the 25th and 75th percentile (interquartile range). It is possible to observe a wide interquartile range in all years, a significant number of outliers on the upper bound and also values close to 0. This suggests the need to address data quality systematically. 
...
•	Difference within schools > 3 SD or < 3 SD – i.e. if the difference in PTR for a school at different points in time are > or <3sd of the whole set of schools, the school is omitted
...
By doing this, we lose 8.3% of the observations. This will not affect statistical power (decrease from 51,786 to 47,778 observations), leaving a panel of 15,926 schools. Figure \@ref(fig:quartiles2) shows school PTR distribution after outliers treatment. Whilst we have not analyzed this further, the figure suggests that the spread of school PTRs inside and outside of the middle 50 percent became larger in almost all regions . 
...
ptrout<-PTR%>% group_by(year)%>% 
  summarise(Schools = n_distinct(id),
            ...
            `Average PTR` = mean(PTR,na.rm = T),
            `Standard Deviation PTR`= sd(PTR,na.rm = T))
...
mout2<- c("Median PTR", "Average PTR","Standard Deviation PTR")
...
share<-ptr_2019_2015.anova%>%group_by(year.ptr)%>%
  summarise(`Median % at LGA level`= median(share.sch.not.accept),
            `Average (%)`= mean(share.sch.not.accept),
            `Standard Deviation % of schools in LGA` = sd(share.sch.not.accept))

share<-flextable(share)

mshare = c("Median % at LGA level", "Average (%)","Standard Deviation % of schools in LGA")
...
•	The red area represents councils both having a surplus and a current PTR over 50. These Councils have teachers that could be redeployed. However, this misallocation of teachers is not excessive – i.e. the surpluses are small in all concerned LGAs. 
...
There is a strong negative linear correlation between PTR and teacher surplus (-.70, -.80 and -.87) across years (see Table \@ref(tab:cora)), suggesting a direct relationship between PTR and teacher surplus. This also can be interpreted as teachers' gap being a strong constraint in terms of decreasing PTR over time, which becomes more stringent in 2019.

In all annual plots, the tails in the negative and positive sides show different patterns. With a PTR of less than 50, found in the left upper quadrant (green), there is a larger variance between councils in terms of teacher surplus, which suggests different levels of efficiency and highly inefficient teacher allocation in a number of Councils with very large surpluses. However, the majority of councils in the yellow and red areas could be considered efficient in terms of teacher allocation due to the lack (yellow) or small size (red) of teacher surplus.
...
While the interaction between $x_{amount_{ij}}$ and $x_{ptr_{ij}}$ is of our main interest as predicting changes on PTR, we perform our analysis starting by studying each term separately and adjusting for each other in Models (1)-(3). Models (1), (3) and (4) show a negative association between DLI amount over time, as expected due to the endogenous relationship between those variables. Model (4) does not show significant interaction between councils with high PTR in 2015 (over 50) and the amount received on each year, which means there are no relevant differences between the two groups (councils with high PTR in 2015 and the others) in terms of the amounts received on the PTR DLI. This tallies with other evidence, that whilst some LGAs made an effort others did not, which appears to have confounded the effect of the LGAs’ starting positions.  
...
We observe a statistically significant negative association of councils with higher PTR in 2015 (high.ptr) with PTR council differences over time (difference.council.ptr), where parameters are between -2.1 and -2.5 (95% CI -4.1, -.5). This represents a diminution on average of 2 pupils per teacher in councils with higher PTR in comparison to the others, controlling for all other variables.  Whilst this suggests that the national/regional level authorities somewhat favored most underserved LGAs in deploying newly recruited teachers, this difference (of 2) must be seen in light of the sharp increase of the national mean PTR (of 11 in 4 years).   
...
Model (9) shows a statistically significant association between the sum of money received by each council for all DLIs but 4.2 until 2018 and a dichotomized variable (high.ptr) that represents councils in 2015 with PTR over 50. Being a council with high PTR predicts a difference of $\$32,726$ $(s.e. \$9,574)$ on the period over the other councils. However, after adding control variables such as the differences of PTR between 2019 and 2015, estimates become non-significant, with slopes not different to 0.
...
payments.with42<-payments %>% group_by(council)%>% filter(DLI != 4.2)%>% filter(Year!=2019)%>%
...
FITA<-lmer(Amount.t ~ high.ptr  + (1|REGION),
...
FITA2<-lmer(Amount.t ~ high.ptr + difference.council.ptr + (1|REGION),
            data=PTR.REGION.COUNCIL.2,REML = T,
           control=lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e4)))

FITA3<-lmer(Amount.t ~ high.ptr *difference.council.ptr  +(1|REGION),
            data=PTR.REGION.COUNCIL.2,REML = T,
           control=lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e4)))

#plot_model(FITA3,ci.lvl = F)

stargazer(FITA,FITA2,FITA3,type = "text",header = F,
          column.labels=c("(9)", "(10)", "(11)"),model.numbers = F)
...
We focus on the analysis of possible linkages between good/bad teacher management (our current measure is LGA-level average in PTR) and primary completion (measured by LGA-level survival rates).  Positive impacts of pupil-teacher ratio are linked to a variety of school outcomes such as primary completion rates. As class sizes get larger, learning opportunities get smaller which drives to higher odds of grade repetition. It is known that grade repetition are significant predictors of schooling dropout. 

All models show an expected negative association between districts with PTR over 50 in 2015 and average survival rates. The association is statistically significant in all cases (p.<.05). There is no evidence that DLI money is associated to survival rates after controlling for differences between districts, as shown in models (12)-(15).
...
stargazer(fit8,fit9,fit10,fit11,type="text",header = F,
          column.labels=c("(12)", "(13)", "(14)","(15)"),model.numbers = F)
...
We focus on national exams: Primary School Leaving Exams (PSLE) and Standard Four National Assessment (SFNA). Table \@ref(tab:ta1) presents the number of schools for both exams across 2014-2019. SFNA 2014 is unavailable; more centers do the earlier exam (SFNA), reflecting the number of early-grade-only schools that have been opened to support recent enrolment increases.
...
We follow the same approach to detect outliers than before, excluding observations with more or less than 3 zscores from mean and differences within schools over time larger than 3SD. Figure \@ref(fig:psle2) shows the relatively small proportion of excluded observations.
...
stargazer(fit16, fit16b, fit17, fit17b,type = "text",
          column.labels=c("(16)", "(17)", "(18)","(19)"),model.numbers = F)

```


At school-level, model (20) shows a positive significant association between differences in PSLE school scores (difference.school) in the period 2014 and 2019 and the aggregated value for DLI payments for the period 2014 to 2018. However, after controlling for average scores, results become not significant in Model (21). The same analysis was performed at council-level, without finding a significant association - see Models (22)-(23).
...
stargazer(fit16.dif, fit16b.dif,fit16.dif2.X, fit16b.dif2.X,type = "text",
           column.labels=c("(20)", "(21)", "(22)","(23)"),model.numbers = F,
...
stargazer(fit18,fit19,fit20,fit21,type = "text",
          column.labels = c("(24)","(25)","(26)","(27)"),model.numbers = F)
...
Models (28)-(31) show regression coefficients in models where we fail to reject the null hypothesis of an association between differences in SFNA school scores in the period 2015 and 2018 and DLI payments, both at school-levels and council-levels.
...
stargazer(fit16.dif.sfna, fit16b.dif.sfna,fit16.dif2.X.sfna, fit16b.dif2.X.sfna,type = "text",
           column.labels=c("(28)", "(29)", "(30)","(31)"),model.numbers = F,
           omit.stat = c("AIC", "f","BIC","ll","ser"))
...
- Inference:
  ...
  - Circumstantial evidence of association of school level scores differences in PSLE and total amount LGA DLI.
