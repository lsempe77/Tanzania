...
ptr_2019_2017_2015 <- read_excel("ptr/ptr_2019_2015_w_pbr_final.xlsx", 
                                 sheet = "ptr_2019_2015", 
                                 col_types = c("text", 
...
``` {r, plotptr, fig.cap="Average PTR and Ideal teacher surplus"}
...
``` {r, pslecouncilmoney, fig.cap="PSLE average of councils grouped by overall DLR received"}
...
``` {r, sfna4, fig.cap="PSLE average of councils grouped by overall DLR received"}
...
Location_details_TZ_Uwezo2015 <- read_dta("uwezo/Location_details_TZ_Uwezo2015.dta")
...
summary(m <- vglm(scale(kiswahili) ~ as.factor(year) + scale(Amount.Total) + scale(grade) + 
             as.factor(sex),
           data=subset(uwezo.payments,grade<11), tobit(Upper = 5)))
